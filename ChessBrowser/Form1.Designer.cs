...
namespace ChessBrowser
{
    partial class Form1
    {
        ...
        private void UploadGamesToDatabase(string PGNfilename)
        {
            ...
            Console.WriteLine("Read games total count: " + reader.games.Count);
            ...
            // SetNumWorkItems(...);
            SetNumWorkItems(reader.games.Count);
            ...
            using (MySqlConnection conn = new MySqlConnection(connection))
            {
                try
                {
                    ...
                    foreach (ChessGame game in reader.games)
                    {
                        ...
                        insertWhitePlayer.Parameters.AddWithValue("@elo", game.WhiteElo);
                        if (playerSearchReader.HasRows)
                        {
                            playerSearchReader.Close();
                            //Update Player
                            insertWhitePlayer.CommandText = "update Players set Elo = @elo where Name = @name;";
                            insertWhitePlayer.ExecuteNonQuery();
                        }
                        else
                        {
                            playerSearchReader.Close();
                            //Insert Player
                            insertWhitePlayer.CommandText = "insert ignore into Players(Name, Elo) values(@Name, @Elo);";
                            insertWhitePlayer.ExecuteNonQuery();
                        }
                        ...
                        insertBlackPlayer.Parameters.AddWithValue("@elo", game.BlackElo);
                        if (playerSearchReader.HasRows)
                        {
                            playerSearchReader.Close();
                            insertBlackPlayer.CommandText = "update Players set Elo = @elo where Name = @name;";
                            insertBlackPlayer.ExecuteNonQuery();
                        }
                        else
                        {
                            playerSearchReader.Close();
                            insertBlackPlayer.CommandText = "insert ignore into Players(Name, Elo) values(@Name, @Elo);";
                            insertBlackPlayer.ExecuteNonQuery();
                        }
                        ...
                        eventSearchCommand.CommandText = "select * from Events where Name = @Name and Site = @Site and Date = @Date;";
                        eventSearchCommand.Parameters.AddWithValue("@Name", game.Event);
                        eventSearchCommand.Parameters.AddWithValue("@Site", game.Site);
                        eventSearchCommand.Parameters.AddWithValue("@Date", game.EventDate);

                        MySqlDataReader eventSearcher = eventSearchCommand.ExecuteReader();

                        MySqlCommand insertEvent = conn.CreateCommand();
                        insertEvent.Parameters.AddWithValue("@Name", game.Event);
                        ...
                        {
                            eventSearcher.Close();
                            insertEvent.CommandText = "insert ignore into Events(Name, Site, Date) values(@Name, @Site, @Date);";
                        ...
                        searchIDEvent.Parameters.AddWithValue("@EventSite", game.Site);
                        searchIDEvent.Parameters.AddWithValue("@EventDate", game.EventDate);
                        MySqlDataReader dataReader = searchIDEvent.ExecuteReader();
                        dataReader.Read();
                        ...
                        searchIDPlayer.Parameters.AddWithValue("@WhitePlayerName", game.White);
                        dataReader = searchIDPlayer.ExecuteReader();
                        dataReader.Read();
                        ...
                        insertGame.CommandText = "insert ignore into Games values(@Round, @Result, @Moves, @BlackPlayer, @WhitePlayer, @eID);";
                        ...
                        insertGame.Parameters.AddWithValue("@BlackPlayer", bID);
                        insertGame.Parameters.AddWithValue("@WhitePlayer", wID);
                        insertGame.Parameters.AddWithValue("@eID", eID);
